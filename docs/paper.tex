...
\STATE Generate Kingman coalescent tree with $N$ leaves and $N_e$
\FOR{$j = 1$ to $M$}
    \STATE Sample branch proportional to length; carriers $\gets$ leaf descendants; $\text{CCF}_j \gets |\text{carriers}|/N$
    \STATE Sample $C_j$, $m_j \sim \text{Uniform}\{1, \ldots, C_j\}$
